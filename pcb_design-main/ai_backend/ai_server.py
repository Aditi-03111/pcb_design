...
LATEST_GENERATION_PATH = OUTPUT_DIR / "latest_generation.json"
...
class JudgeBundleResponse(BaseModel):
    success:            bool
    request_id:         str
    title:              str
    prompt:             str
...
class KiCadIntegrationStatus(BaseModel):
    available:          bool
    prompt:             Optional[str] = None
    schematic_file:     Optional[str] = None
    schematic_path:     Optional[str] = None
    schematic_download_url: Optional[str] = None
    bundle_file:        Optional[str] = None
    bundle_download_url: Optional[str] = None
...
class KiCadOpenResponse(BaseModel):
    success: bool
...
_DEMO_PROMPTS: Dict[str, str] = {
    "555_timer": "555 timer astable LED blinker at 1Hz",
    "3v3_regulator": "3.3V regulator from 5V input with decoupling capacitor",
    "led_resistor": "LED resistor indicator from 5V supply",
    "mosfet_switch": "N-channel MOSFET low-side switch for a DC load",
    "opamp_buffer": "Op-amp voltage follower buffer for a sensor input",
}


def _slugify_filename(value: str, fallback: str) -> str:
    ascii_value = value.encode("ascii", errors="ignore").decode("ascii")
    safe = re.sub(r"[^\w\s\-]", "", ascii_value).strip()
    safe = re.sub(r"\s+", "_", safe)
    safe = re.sub(r"_+", "_", safe).strip("_")
    return safe[:80] or fallback


def _summarize_prompt(prompt: str, template_used: Optional[str]) -> str:
    prompt_clean = " ".join(prompt.split())
    if template_used:
        return f"Template-backed circuit generated for '{prompt_clean}' using '{template_used}'."
    return f"AI-assisted circuit generated for '{prompt_clean}'."


def _build_bom_csv(board: BoardData) -> str:
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Reference", "Value", "Footprint", "Layer", "Polarized"])
    for comp in board.components:
        writer.writerow([
            comp.ref,
            comp.value,
            comp.footprint,
            comp.layer,
            "yes" if comp.is_polarized else "no",
        ])
    return output.getvalue()


def _build_preview_svg(board: BoardData, prompt: str) -> str:
    width, height = 960, 640
    margin = 90
    usable_w = width - margin * 2
    usable_h = height - margin * 2
    components = [c for c in board.components if not c.is_power_symbol]
    board_w = max(board.board_width, 1.0)
    board_h = max(board.board_height, 1.0)

    def sx(x: float) -> float:
        return margin + (x / board_w) * usable_w

    def sy(y: float) -> float:
        return margin + (y / board_h) * usable_h
    ...
    for conn in board.connections:
        ...
        if len(refs) < 2:
            continue
        anchor = board.get_component(refs[0])
        if not anchor:
            continue
        x1, y1 = sx(anchor.x), sy(anchor.y)
        for ref in refs[1:]:
            comp = board.get_component(ref)
            if not comp:
                continue
            traces.append(
                f'<line x1="{x1:.1f}" y1="{y1:.1f}" x2="{sx(comp.x):.1f}" y2="{sy(comp.y):.1f}" '
                'stroke="rgba(255,195,0,0.22)" stroke-width="2" />'
            )

    for comp in components:
        x, y = sx(comp.x), sy(comp.y)
        label = f"{comp.ref} {comp.value}".strip()
        nodes.append(
            f'<g><circle cx="{x:.1f}" cy="{y:.1f}" r="16" fill="#f25f4c" />'
            f'<circle cx="{x:.1f}" cy="{y:.1f}" r="28" fill="none" stroke="rgba(242,95,76,0.22)" stroke-width="2" />'
            f'<text x="{x:.1f}" y="{y + 44:.1f}" fill="#f8f4e8" font-size="12" text-anchor="middle">{label}</text></g>'
        )
...
def _bundle_files_for_board(
    *,
    ...
    existing_schematic: Optional[str] = None,
) -> JudgeBundleResponse:
    dfm_violations = AdvancedDFMEngine(board).analyze()
    critical = sum(1 for v in dfm_violations if v.severity in ("critical", "error"))
    warning_count = sum(1 for v in dfm_violations if v.severity == "warning")
    info_count = sum(1 for v in dfm_violations if v.severity == "info")
    penalty = critical * 18 + warning_count * 8 + info_count * 3
    dfm_score = max(40, 100 - penalty)
    ...
    title = _slugify_filename(prompt, request_id).replace("_", " ").title()
    summary = _summarize_prompt(prompt, template_used)
    top_issues = [v.message for v in dfm_violations[:5]]

    schematic_filename = existing_schematic
        ...
        try:
            ...
            circuit_payload = {
                ...
                "connections": [
                    {
                        "net": conn.net,
                        "pins": [str(pin) for pin in conn.pins],
                    }
                    for conn in board.connections
                ],
            }
            schematic_content = export_to_kicad_sch(CircuitData(**circuit_payload))
            schematic_filename = f"{_slugify_filename(prompt, request_id)}.kicad_sch"
            (OUTPUT_DIR / schematic_filename).write_text(schematic_content, encoding="utf-8")
        except Exception as exc:
            warnings = [*warnings, f"KiCad export unavailable for bundle: {exc}"]
    ...
    manifest = {
        ...
        "net_count": len(board.connections),
        "dfm_score": dfm_score,
    ...
    bom_filename = f"{_slugify_filename(prompt, request_id)}_bom.csv"
    json_filename = f"{_slugify_filename(prompt, request_id)}.json"
    preview_filename = f"{_slugify_filename(prompt, request_id)}_preview.svg"
    report_filename = f"{_slugify_filename(prompt, request_id)}_judge_report.md"
    bundle_filename = f"{_slugify_filename(prompt, request_id)}_bundle.zip"

    preview_svg = _build_preview_svg(board, prompt)
    report_lines = [
        f"# {title}",
        "",
        summary,
        ...
        f"- Nets: {len(board.connections)}",
        f"- DFM score: {dfm_score}/100",
        f"- Risk level: {risk_level}",
        f"- Template used: {template_used or 'none'}",
        "",
        "## Top Issues",
    ]
    if top_issues:
        report_lines.extend([f"- {issue}" for issue in top_issues])
    else:
        report_lines.append("- No blocking DFM issues detected.")

    bundle_path = OUTPUT_DIR / bundle_filename
    with zipfile.ZipFile(bundle_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr(json_filename, json.dumps(board.model_dump(), indent=2))
        archive.writestr("manifest.json", json.dumps(manifest, indent=2))
        archive.writestr(bom_filename, _build_bom_csv(board))
        archive.writestr(preview_filename, preview_svg)
        archive.writestr(report_filename, "\n".join(report_lines))
        if schematic_filename and (OUTPUT_DIR / schematic_filename).exists():
            archive.write(OUTPUT_DIR / schematic_filename, arcname=schematic_filename)

    files = {
        "json": f"/download/{json_filename}",
        "bom": f"/download/{bom_filename}",
        "preview": f"/download/{preview_filename}",
        "report": f"/download/{report_filename}",
    }
    if schematic_filename:
        files["schematic"] = f"/download/{schematic_filename}"

    (OUTPUT_DIR / json_filename).write_text(json.dumps(board.model_dump(), indent=2), encoding="utf-8")
    (OUTPUT_DIR / bom_filename).write_text(_build_bom_csv(board), encoding="utf-8")
    (OUTPUT_DIR / preview_filename).write_text(preview_svg, encoding="utf-8")
    (OUTPUT_DIR / report_filename).write_text("\n".join(report_lines), encoding="utf-8")
    _record_latest_generation({
        "request_id": request_id,
        "prompt": prompt,
        "schematic_file": schematic_filename,
        "schematic_path": str((OUTPUT_DIR / schematic_filename).resolve()) if schematic_filename else None,
        "schematic_download_url": f"/download/{schematic_filename}" if schematic_filename else None,
        "bundle_file": bundle_filename,
        "bundle_download_url": f"/download/{bundle_filename}",
        "generated_at": time.strftime("%Y-%m-%dT%H:%M:%S"),
    })

    return JudgeBundleResponse(
        success=True,
        request_id=request_id,
        title=title,
        prompt=prompt,
...
def _record_latest_generation(payload: Dict[str, Any]) -> None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    LATEST_GENERATION_PATH.write_text(json.dumps(payload, indent=2), encoding="utf-8")


def _load_latest_generation() -> Optional[Dict[str, Any]]:
    if not LATEST_GENERATION_PATH.exists():
        return None
    try:
        return json.loads(LATEST_GENERATION_PATH.read_text(encoding="utf-8"))
    except Exception:
        logger.warning("Failed to read latest generation metadata", exc_info=True)
        return None
...
async def generate_circuit(
    ...
    _record_latest_generation({
        ...
        "schematic_file": sch_filename,
        "schematic_path": str((OUTPUT_DIR / sch_filename).resolve()) if sch_filename else None,
        "schematic_download_url": download_url,
        "bundle_file": None,
...
@app.post("/generate/judge", response_model=JudgeBundleResponse, tags=["generation"])
async def generate_judge_bundle(request: GenerateRequest) -> JudgeBundleResponse:
    """Generate a circuit plus a demo-friendly bundle for judges."""
    result = await generate_circuit(request, BackgroundTasks())
    if not result.success or not result.circuit_data:
        return JudgeBundleResponse(
            success=False,
            request_id=result.request_id,
            title="Generation failed",
            prompt=request.prompt,
            summary="Circuit generation failed.",
            warnings=result.warnings,
            error=result.error,
        )

    board = BoardData(**result.circuit_data)
    return _bundle_files_for_board(
        board=board,
        request_id=result.request_id,
        prompt=request.prompt,
        warnings=result.warnings,
        ...
        existing_schematic=result.output_file,
    )


@app.get("/demo-circuits", tags=["templates"])
async def list_demo_circuits() -> List[Dict[str, Any]]:
    """Expose stable demo circuits for the frontend judge flow."""
    demos: List[Dict[str, Any]] = []
    for name, prompt in _DEMO_PROMPTS.items():
        template = _state.template_cache.get(name, {})
        demos.append({
            "name": name,
            "title": name.replace("_", " ").title(),
            "prompt": prompt,
            "description": template.get("description", prompt),
            "component_count": len(template.get("components", [])),
            "net_count": len(template.get("connections", [])),
            "category": template.get("metadata", {}).get("category", "starter"),
        })
    return demos


@app.post("/demo/{name}/bundle", response_model=JudgeBundleResponse, tags=["templates"])
async def build_demo_bundle(name: str) -> JudgeBundleResponse:
    safe = re.sub(r"[^a-zA-Z0-9_\-]", "", name)
    if safe not in _DEMO_PROMPTS:
        raise HTTPException(status_code=404, detail=f"Demo '{safe}' not found.")
    return await generate_judge_bundle(
        GenerateRequest(prompt=_DEMO_PROMPTS[safe], priority="speed")
    )


@app.get("/integration/kicad/latest", response_model=KiCadIntegrationStatus, tags=["integration"])
async def get_latest_kicad_generation() -> KiCadIntegrationStatus:
    latest = _load_latest_generation()
    if not latest:
        return KiCadIntegrationStatus(
            available=False,
            error="No generated schematic is available yet.",
        )
    return KiCadIntegrationStatus(
        available=bool(latest.get("schematic_path")),
        prompt=latest.get("prompt"),
        schematic_file=latest.get("schematic_file"),
        schematic_path=latest.get("schematic_path"),
        schematic_download_url=latest.get("schematic_download_url"),
        bundle_file=latest.get("bundle_file"),
        bundle_download_url=latest.get("bundle_download_url"),
        request_id=latest.get("request_id"),
        generated_at=latest.get("generated_at"),
        error=None if latest.get("schematic_path") else "Latest generation has no schematic file.",
    )


@app.post("/integration/kicad/open-latest", response_model=KiCadOpenResponse, tags=["integration"])
async def open_latest_in_kicad() -> KiCadOpenResponse:
    latest = _load_latest_generation()
    if not latest:
        return KiCadOpenResponse(
            success=False,
            message="No generated schematic is available yet.",
    ...
    if not schematic_path:
        return KiCadOpenResponse(
            success=False,
            message="Latest generation does not include a schematic path.",
            error="missing_schematic",
        )
    ...
    try:
        ...
        logger.warning("Failed to open schematic in system default app: %s", exc)
        return KiCadOpenResponse(
            success=False,
            message="Failed to open the latest schematic in KiCad.",
            error=str(exc),
        )
    ...
    return KiCadOpenResponse(
        ...
        message="Opened the latest generated schematic with the system default application.",
    )
...
async def download_file(filename: str) -> FileResponse:
    ...
    if not safe.endswith((".kicad_sch", ".json", ".csv", ".svg", ".md", ".zip")):
        raise HTTPException(
    ...
    return [
        {
            ...
            "net_count":   len(data.get("connections", [])),
