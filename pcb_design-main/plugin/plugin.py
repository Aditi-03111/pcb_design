...
class AIPCBFrame(wx.Frame):
    ...
    def _init_ui(self):
        ...
        toolbar.AddTool(104, "Open Web Schematic", wx.ArtProvider.GetBitmap(wx.ART_FILE_OPEN), "Open Latest Web Schematic")
        toolbar.Realize()
        ...
        self.Bind(wx.EVT_TOOL, self._on_open_latest_web_schematic, id=104)
    ...
    def _create_left_panel(self, parent) -> wx.Panel:
        ...
        btn_open_web = wx.Button(panel, label="Open Latest Web Schematic")
        btn_open_web.Bind(wx.EVT_BUTTON, self._on_open_latest_web_schematic)
        box_sizer.Add(btn_open_web, 0, wx.ALIGN_RIGHT | wx.LEFT | wx.RIGHT | wx.BOTTOM, 5)
    ...
    def _on_open_latest_web_schematic(self, event):
        """Open the most recent schematic generated from the web UI."""
        try:
            status = self._fetch_json(f"{CONFIG.backend_url}/integration/kicad/latest")
        except Exception as exc:
            wx.MessageBox(
                f"Could not reach the KiCad integration endpoint.\n\n{exc}",
                "KiCad Integration Error",
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        if not status.get("available"):
            wx.MessageBox(
                status.get("error", "No generated schematic is available yet."),
                "No Web Schematic Found",
                wx.OK | wx.ICON_INFORMATION,
            )
            return

        schematic_path = status.get("schematic_path", "")
        if schematic_path and os.path.exists(schematic_path):
            if wx.LaunchDefaultApplication(schematic_path):
                self.info_text.SetValue(
                    "Opened latest web schematic.\n\n"
                    f"Prompt: {status.get('prompt', 'n/a')}\n"
                    f"File: {schematic_path}"
                )
                self.SetStatusText("Opened latest web schematic")
                return

        try:
            result = self._post_json(f"{CONFIG.backend_url}/integration/kicad/open-latest")
        except Exception as exc:
            wx.MessageBox(
                f"Failed to open the latest schematic.\n\n{exc}",
                "KiCad Integration Error",
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        if result.get("success"):
            self.info_text.SetValue(
                "Opened latest web schematic via backend handoff.\n\n"
                f"Prompt: {status.get('prompt', 'n/a')}\n"
                f"File: {result.get('opened_path', 'n/a')}"
            )
            self.SetStatusText("Opened latest web schematic")
            return

        wx.MessageBox(
            result.get("message", "Failed to open the latest schematic."),
            "KiCad Integration Error",
            wx.OK | wx.ICON_ERROR,
        )
